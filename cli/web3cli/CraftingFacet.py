...
class CraftingFacet:
    ...
    def add_recipe(self, recipe: tuple, transaction_config) -> Any:
        self.assert_contract_is_instantiated()
    ...
    def set_terminus_auth(
        self, terminus_address: ChecksumAddress, token_id: int, transaction_config
    ) -> Any:
        self.assert_contract_is_instantiated()
        return self.contract.setTerminusAuth(
            terminus_address, token_id, transaction_config
        )
    ...
    def update_recipe(self, recipe_id: int, recipe: tuple, transaction_config) -> Any:
        self.assert_contract_is_instantiated()
        return self.contract.updateRecipe(recipe_id, recipe, transaction_config)
...
def handle_set_terminus_auth(args: argparse.Namespace) -> None:
    network.connect(args.network)
    contract = CraftingFacet(args.address)
    transaction_config = get_transaction_config(args)
    result = contract.set_terminus_auth(
...
def handle_update_recipe(args: argparse.Namespace) -> None:
    network.connect(args.network)
    contract = CraftingFacet(args.address)
    transaction_config = get_transaction_config(args)
    result = contract.update_recipe(
...
def generate_cli() -> argparse.ArgumentParser:
    ...
    add_recipe_parser.add_argument(
        "--recipe", required=True, help="Type: tuple", type=eval
    )
    ...
    set_terminus_auth_parser = subcommands.add_parser("set-terminus-auth")
    add_default_arguments(set_terminus_auth_parser, True)
    set_terminus_auth_parser.add_argument(
        "--terminus-address", required=True, help="Type: address"
    )
    set_terminus_auth_parser.add_argument(
        "--token-id", required=True, help="Type: uint256", type=int
    )
    set_terminus_auth_parser.set_defaults(func=handle_set_terminus_auth)
    ...
    update_recipe_parser = subcommands.add_parser("update-recipe")
    add_default_arguments(update_recipe_parser, True)
    update_recipe_parser.add_argument(
        "--recipe-id", required=True, help="Type: uint256", type=int
    )
    update_recipe_parser.add_argument(
        "--recipe", required=True, help="Type: tuple", type=eval
    )
    update_recipe_parser.set_defaults(func=handle_update_recipe)
