...
class AchievementFacet:
    def __init__(self, contract_address: Optional[ChecksumAddress]):
        self.contract_name = "AchievementFacet"
        self.address = contract_address
        self.contract = None
        self.abi = get_abi_json("AchievementFacet")
    ...
    def create_achievement_slot(self, metadata_uri: str, transaction_config) -> Any:
        self.assert_contract_is_instantiated()
    ...
    def grant_admin_privilege(self, admin: ChecksumAddress, transaction_config) -> Any:
        self.assert_contract_is_instantiated()
        return self.contract.grantAdminPrivilege(admin, transaction_config)
    ...
    def initialize(self, subject_address: ChecksumAddress, transaction_config) -> Any:
        self.assert_contract_is_instantiated()
        return self.contract.initialize(subject_address, transaction_config)
    ...
    def revoke_admin_privilege(self, admin: ChecksumAddress, transaction_config) -> Any:
        self.assert_contract_is_instantiated()
        return self.contract.revokeAdminPrivilege(admin, transaction_config)
...
def handle_deploy(args: argparse.Namespace) -> None:
    ...
    contract = AchievementFacet(None)
...
def handle_create_achievement_slot(args: argparse.Namespace) -> None:
    network.connect(args.network)
    contract = AchievementFacet(args.address)
    transaction_config = get_transaction_config(args)
    result = contract.create_achievement_slot(
        metadata_uri=args.metadata_uri, transaction_config=transaction_config
    )
    print(result)
    if args.verbose:
        print(result.info())
...
def handle_grant_admin_privilege(args: argparse.Namespace) -> None:
    network.connect(args.network)
    contract = AchievementFacet(args.address)
    transaction_config = get_transaction_config(args)
    result = contract.grant_admin_privilege(
        admin=args.admin, transaction_config=transaction_config
    )
    print(result)
    if args.verbose:
        print(result.info())
...
def handle_initialize(args: argparse.Namespace) -> None:
    network.connect(args.network)
    contract = AchievementFacet(args.address)
    transaction_config = get_transaction_config(args)
    result = contract.initialize(
        subject_address=args.subject_address, transaction_config=transaction_config
    )
    print(result)
    if args.verbose:
        print(result.info())
...
def handle_revoke_admin_privilege(args: argparse.Namespace) -> None:
    network.connect(args.network)
    contract = AchievementFacet(args.address)
    transaction_config = get_transaction_config(args)
    result = contract.revoke_admin_privilege(
        admin=args.admin, transaction_config=transaction_config
    )
    print(result)
    if args.verbose:
...
def generate_cli() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="CLI for AchievementFacet")
    ...
    create_achievement_slot_parser = subcommands.add_parser("create-achievement-slot")
    add_default_arguments(create_achievement_slot_parser, True)
    create_achievement_slot_parser.add_argument(
        "--metadata-uri", required=True, help="Type: string", type=str
    )
    create_achievement_slot_parser.set_defaults(func=handle_create_achievement_slot)
    ...
    grant_admin_privilege_parser = subcommands.add_parser("grant-admin-privilege")
    add_default_arguments(grant_admin_privilege_parser, True)
    grant_admin_privilege_parser.add_argument(
        "--admin", required=True, help="Type: address"
    )
    grant_admin_privilege_parser.set_defaults(func=handle_grant_admin_privilege)
    ...
    initialize_parser = subcommands.add_parser("initialize")
    add_default_arguments(initialize_parser, True)
    initialize_parser.add_argument(
        "--subject-address", required=True, help="Type: address"
    )
    initialize_parser.set_defaults(func=handle_initialize)
    ...
    revoke_admin_privilege_parser = subcommands.add_parser("revoke-admin-privilege")
    add_default_arguments(revoke_admin_privilege_parser, True)
    revoke_admin_privilege_parser.add_argument(
        "--admin", required=True, help="Type: address"
    )
    revoke_admin_privilege_parser.set_defaults(func=handle_revoke_admin_privilege)
