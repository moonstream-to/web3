...
def handle_create_metagame_equipment(args: argparse.Namespace) -> None:
    ...
    equipment = MockTerminus.MockTerminus(args.equipment_address)
    print("Started with " + str(equipment.total_pools()) + " pools.")

    for item in METAGAME_EQUIPMENT:
        print(item["id"])
        # equipment.create_pool_v1(MAX_UINT, True, True, transaction_config)
        # pool_id = equipment.total_pools()
        # equipment.set_uri(pool_id, item["metadata"], transaction_config)
...
def generate_cli():
    ...
    MockTerminus.add_default_arguments(create_metagame_equipment_parser, transact=True)
