...
class InventoryFacet:
    ...
    def admin_info(self, block_number: Optional[Union[str, int]] = "latest") -> Any:
        self.assert_contract_is_instantiated()
        return self.contract.adminInfo.call(block_identifier=block_number)

    def create_slot(
        self, is_persistent: bool, slot_uri: str, transaction_config
    ) -> Any:
        self.assert_contract_is_instantiated()
        return self.contract.createSlot(is_persistent, slot_uri, transaction_config)
    ...
    def equip_batch(
        ...
        self.assert_contract_is_instantiated()
        return self.contract.equipBatch(
    ...
    def init(
        ...
        self.assert_contract_is_instantiated()
        return self.contract.init(admin_address, contract_address, transaction_config)
    ...
    def mark_item_as_equippable_in_slot(
        ...
        return self.contract.markItemAsEquippableInSlot(
            slot, item_type, item_address, item_token_id, max_amount, transaction_config
        )
    ...
    def max_amount_of_item_in_slot(
        ...
        return self.contract.maxAmountOfItemInSlot.call(
            slot, item_type, item_address, item_token_id, block_identifier=block_number
        )
    ...
    def set_slot_persistent(
        self, slot_id: int, is_persistent: bool, transaction_config
    ) -> Any:
        self.assert_contract_is_instantiated()
        return self.contract.setSlotPersistent(
            slot_id, is_persistent, transaction_config
        )
...
def handle_admin_info(args: argparse.Namespace) -> None:
    network.connect(args.network)
    contract = InventoryFacet(args.address)
    result = contract.admin_info(block_number=args.block_number)
    print(result)
...
def handle_equip_batch(args: argparse.Namespace) -> None:
    network.connect(args.network)
    contract = InventoryFacet(args.address)
    transaction_config = get_transaction_config(args)
    result = contract.equip_batch(
        subject_token_id=args.subject_token_id,
        slots=args.slots,
        items=args.items,
        transaction_config=transaction_config,
...
def generate_cli() -> argparse.ArgumentParser:
    ...
    admin_info_parser = subcommands.add_parser("admin-info")
    add_default_arguments(admin_info_parser, False)
    admin_info_parser.set_defaults(func=handle_admin_info)
    ...
    create_slot_parser.add_argument(
        "--is-persistent", required=True, help="Type: bool", type=boolean_argument_type
    )
    create_slot_parser.add_argument(
    ...
    equip_batch_parser = subcommands.add_parser("equip-batch")
    add_default_arguments(equip_batch_parser, True)
    equip_batch_parser.add_argument(
        "--subject-token-id", required=True, help="Type: uint256", type=int
    )
    equip_batch_parser.add_argument(
    ...
    equip_batch_parser.set_defaults(func=handle_equip_batch)
    ...
    init_parser.add_argument("--admin-address", required=True, help="Type: address")
